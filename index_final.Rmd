...
va_cluster_matrix <- ggpairs(data = va_county_clust
...
ggarrange(
  va_geo,                # First row with line plot
...
vt_cluster_matrix <- ggpairs(data = vt_county_clust
        , aes(color = clust3_std) 
        , columns = vars_std
        , upper = list(continuous = "blank"))
...
ggarrange(
  vt_geo,                # First row with line plot
  # Second row with box and dot plots
...
tn_cluster_matrix <- ggpairs(data = tn_county_clust
        , aes(color = clust3_std) 
        , columns = vars_std
        , upper = list(continuous = "blank"))
...
ggarrange(
  tn_geo,                # First row with line plot
  # Second row with box and dot plots
...
ia_county_clust <- ia_final_county %>%
  mutate(clust3_std = as.character(ia_county_km$cluster)) %>%
  arrange(clust3_std) %>%
  mutate(County=tolower(County))
...
ia_geo <- ggplot() +
  geom_polygon(data = usa_states, aes(x=long, y = lat, group = group), color="black", fill="black", alpha=0.3) +
  geom_polygon(data=ia_final_cluster, aes(x = long, y = lat, group=group, fill=clust3_std), color="black") +
  scale_size_continuous(range=c(1,12)) +
  scale_color_viridis(trans="log") +
...
ia_cluster_matrix <- ggpairs(data = ia_county_clust
        , aes(color = clust3_std) 
        , columns = vars_std
        , upper = list(continuous = "blank"))
...
ggarrange(
  ia_geo,                # First row with line plot
  # Second row with box and dot plots
...
ca_cluster_matrix <- ggpairs(data = ca_county_clust
        , aes(color = clust3_std) 
        , columns = vars_std
        , upper = list(continuous = "blank"))
...
ggarrange(
  ca_geo,                # First row with line plot
  # Second row with box and dot plots
...
##ADD DIVISION FOR EACH STATE

kable(final_counties, booktabs = TRUE) %>%
