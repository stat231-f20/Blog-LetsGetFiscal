...
az_counties <- map_data(map = "county"
                      , region = "arizona")

city_count_star_az <- us_map_star %>%
  left_join(city_star, by = "City") %>%
  filter(State == "AZ")

ggplot() +
  geom_polygon(data = az_counties, aes(x=long, y = lat, group = group), color="black", fill="black", alpha=0.3) +
  geom_point(data=city_count_star_az, aes(x = Longitude, y = Latitude, group=group, size=Count, color=Count)) +
  scale_size_continuous(range=c(1,12)) +
  scale_color_viridis(trans="log") +
  theme_void() 

me_counties <- map_data(map = "county"
                      , region = "maine")

city_count_star_me <- us_map_star %>%
  left_join(city_star, by = "City") %>%
  filter(State == "ME")

ggplot() +
  geom_polygon(data = me_counties, aes(x=long, y = lat, group = group), color="black", fill="black", alpha=0.3) +
  geom_point(data=city_count_star_me, aes(x = Longitude, y = Latitude, group=group, size=Count, color=Count)) +
  scale_size_continuous(range=c(1,12)) +
  scale_color_viridis(trans="log") +
  theme_void() 

ms_counties <- map_data(map = "county"
                      , region = "mississippi")

city_count_star_ms <- us_map_star %>%
  left_join(city_star, by = "City") %>%
  filter(State == "MS")

ggplot() +
  geom_polygon(data = ms_counties, aes(x=long, y = lat, group = group), color="black", fill="black", alpha=0.3) +
  geom_point(data=city_count_star_ms, aes(x = Longitude, y = Latitude, group=group, size=Count, color=Count)) +
  scale_size_continuous(range=c(1,12)) +
  scale_color_viridis(trans="log") +
  theme_void() 
