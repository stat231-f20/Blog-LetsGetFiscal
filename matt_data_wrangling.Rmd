...
#MAINE: Clustering by Unemployment, Starbucks density, and Income of each county
...
ggplot() +
  geom_polygon(data = usa_states, aes(x=long, y = lat, group = group), color="black", fill="black", alpha=0.3) +
  geom_polygon(data=final_cluster, aes(x = long, y = lat, group=group, fill=clust3_std)) +
  scale_size_continuous(range=c(1,12)) +
  scale_color_viridis(trans="log") +
  labs(title = "Clusters in Maine",
       caption = "*Grey states have no tweet information") +
  theme_void()
