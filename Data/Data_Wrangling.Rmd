...
usa_states0 <- map_data(map = "county"
                       , region = ".")
...
county_population <- clean_county %>%
  inner_join(usa_states, by = c("County" = "subregion"))

ggplot(county_population, aes(x = long, y = lat, group = group
                      , fill = population)) +
  geom_polygon(color = "white") +
  theme_void() +
  coord_fixed(ratio = 1.3) +
  labs(fill = "Population Per County") +
  theme(legend.position="bottom")
