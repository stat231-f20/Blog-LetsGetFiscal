...
Based on the cluster analysis, the total within-cluster sum of squares seems to significantly diminish after 5 clusters. Therefore, in our evaluation of Virginia, the state was divided into 5 clusters. To best contribute to a more socially-minded Starbucks, we believe the counties a part of Cluster 1 are best. The counties in Cluster 1 have the lowest household mean income and highest unemployment rates relative to the other clusters, and also has a low amount of Starbucks stores. Establishing a Starbucks location in one of the counties of Cluster 1 will provide more job opportunties and support for a community that, according to the factors selected, need it.
...
Based on the cluster analysis, the total within-cluster sum of squares seems to significantly diminish after 5 clusters. Therefore, in our evaluation of Iowa, the state was divided into 4 clusters. To best contribute to a more socially-minded Starbucks, we believe the counties a part of Cluster 4 are best. While the counties in Cluster 4 have an average household mean income, the counties do have the highest unemployment rates relative to the other clusters and also has a low amount of Starbucks stores. Establishing a Starbucks location in one of the counties of Cluster 4 will provide more job opportunties and support for a community that, according to the factors selected, need it due to the relatively low unemployment rate.
...
final_ia_poor_county <- ia_poor_county %>%
  mutate(State="Iowa") %>%
...
final_tn_poor_county <- tn_poor_county %>%
...
final_counties <- final_ca_poor_county %>%
  bind_rows(final_ia_poor_county) %>%
  bind_rows(final_vt_poor_county) %>%
  bind_rows(final_va_poor_county) %>%
  bind_rows(final_tn_poor_county) 
